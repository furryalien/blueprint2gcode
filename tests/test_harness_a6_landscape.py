#!/usr/bin/env python3
"""
Test harness for blueprint2gcode with A6 paper size in LANDSCAPE orientation
Processes test images and visualizes input vs G-code output for A6 landscape format
"""
...
def visualize_comparison(input_image, gcode_file, title, output_image):
    ...
    # Plot G-code output (left) - A6 LANDSCAPE is 148mm wide x 105mm high
    ax1.set_aspect('equal')
    ax1.set_title('G-code Output (A6 Landscape)', fontsize=14, fontweight='bold')
    ax1.set_xlabel('X (mm)')
    ax1.set_ylabel('Y (mm)')
    ax1.grid(True, alpha=0.3)
    
    # Draw A6 paper boundary in landscape (148 x 105 mm)
    paper_rect = patches.Rectangle((0, 0), 148, 105, linewidth=2, 
                                   edgecolor='lightgray', facecolor='none',
                                   linestyle='--', label='A6 Landscape (148×105mm)')
    ...
    # Set axis limits to show full A6 landscape paper
    ax1.set_xlim(-5, 153)
    ax1.set_ylim(-5, 110)
    ax1.legend()
...
def run_conversion(input_file, output_file):
    """Run blueprint2gcode conversion with A6 paper size in LANDSCAPE orientation."""
    cmd = [
        sys.executable, 
        'blueprint2gcode.py',
        str(input_file),
    ...
    print(f"\n{'='*70}")
    print(f"Processing: {input_file.name} (A6 Landscape)")
    print(f"{'='*70}")
...
def create_summary_html(../test_data/test_results, output_dir):
    """Create an HTML summary page with all A6 landscape visualizations."""
    html = """<!DOCTYPE html>
<html>
<head>
    <title>Blueprint2GCode A6 Landscape Test Results</title>
    <style>
        body {
...
<body>
    ...
    <div class="paper-info">
        ...
        <div class="orientation-badge">📐 LANDSCAPE ORIENTATION</div>
        <p style="margin-top: 15px;">Dimensions: 148mm × 105mm (5.8" × 4.1")</p>
        <p>All images forced to landscape orientation with automatic rotation</p>
    </div>
    <div class="header-info">
        Generated on: """ + str(Path.cwd()) + """<br>
        Total test cases: """ + str(len(../test_data/test_results)) + """
    </div>
"""
    
    for test_name, viz_file in ../test_data/test_results:
        html += f"""
...
"""
    ...
    <div class="footer">
        ...
        <p>All outputs in A6 Landscape format (148mm × 105mm)</p>
...
"""
    ...
    html_file = output_dir / '../test_data/test_results_a6_landscape.html'
...
def main():
    ...
    output_dir = Path('../test_data/test_output_a6_landscape')
    output_dir.mkdir(exist_ok=True)
    
    viz_dir = Path('../visualizations/test_visualizations_a6_landscape')
    ...
    print(f"Paper size: A6 Landscape (148mm × 105mm)")
    print(f"Orientation: LANDSCAPE (forced)")
    ...
    for test_image in test_images:
        output_file = output_dir / f"{test_image.stem}_a6_landscape.gcode"
        viz_file = viz_dir / f"{test_image.stem}_a6_landscape_comparison.png"
        ...
        try:
            ...
            ../test_data/test_results.append((test_image.stem, viz_file))
    ...
    # Create HTML summary
    if ../test_data/test_results:
        html_file = create_summary_html(../test_data/test_results, viz_dir)
        print(f"\n{'='*70}")
