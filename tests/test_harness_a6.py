#!/usr/bin/env python3
"""
Test harness for blueprint2gcode with A6 paper size
Processes test images and visualizes input vs G-code output for A6 format
"""
...
def visualize_comparison(input_image, gcode_file, title, output_image):
    ...
    # Plot G-code output (left)
    ax1.set_aspect('equal')
    ax1.set_title('G-code Output (A6 Size)', fontsize=14, fontweight='bold')
    ax1.set_xlabel('X (mm)')
    ax1.set_ylabel('Y (mm)')
    ax1.grid(True, alpha=0.3)
    
    # Draw A6 paper boundary (105 x 148 mm)
    paper_rect = patches.Rectangle((0, 0), 105, 148, linewidth=2, 
                                   edgecolor='lightgray', facecolor='none',
                                   linestyle='--', label='A6 Paper (105×148mm)')
    ...
    # Set axis limits to show full A6 paper
    ax1.set_xlim(-5, 110)
    ax1.set_ylim(-5, 153)
    ax1.legend()
...
def run_conversion(input_file, output_file):
    """Run blueprint2gcode conversion with A6 paper size."""
    cmd = [
        sys.executable, 
        'blueprint2gcode.py',
        str(input_file),
    ...
    print(f"\n{'='*70}")
    print(f"Processing: {input_file.name} (A6 format)")
    print(f"{'='*70}")
...
def create_summary_html(../test_data/test_results, output_dir):
    """Create an HTML summary page with all A6 visualizations."""
    html = """<!DOCTYPE html>
<html>
<head>
    <title>Blueprint2GCode A6 Test Results</title>
    <style>
        body {
...
<body>
    ...
    <div class="paper-info">
        ...
        <p>Dimensions: 105mm × 148mm (4.1" × 5.8")</p>
        <p>Perfect for compact drawings, notebooks, and postcards</p>
    </div>
...
"""
    ...
    html_file = output_dir / '../test_data/test_results_a6.html'
...
def main():
    ...
    output_dir = Path('../test_data/test_output_a6')
    output_dir.mkdir(exist_ok=True)
    
    viz_dir = Path('../visualizations/test_visualizations_a6')
    ...
    print(f"Paper size: A6 (105mm × 148mm)")
    ...
    for test_image in test_images:
        output_file = output_dir / f"{test_image.stem}_a6.gcode"
        viz_file = viz_dir / f"{test_image.stem}_a6_comparison.png"
